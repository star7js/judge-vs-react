...
\textbf{action-repeat does not rescue it}; (iv) the boundary is a
...
($\rstar = 0.98$); and (v) a \textbf{real model traces the same boundary} --- a
closed-loop Sonnet agent, swept through the transition by injected delay with
its own frame-by-frame judgment in the loop, collapses at $\rstar = 1.05$, and
past the boundary the \emph{only} surviving episodes are ones that misjudged
early, their perception error cancelled by the delay. Measuring five real
vision models on the task, \textbf{all
...
\begin{itemize}
  ...
  \item \textbf{S5 (a real model through the boundary):} a closed-loop Sonnet
    agent, judging every frame itself, swept through the transition by
    injected delay collapses at $\rstar = 1.05$ --- and past the boundary its
    only clears are \emph{delay-rescued misjudgments}, the two failure axes
    cancelling.
\end{itemize}
...
\subsection{A real model through the boundary (S5)}

S1 measures real models' latencies and finds them all far past $\rstar$
($r \approx 4$--$47$); the transition itself is mapped by the oracle. Here we
close that gap: a real model is swept \emph{through} the boundary with its own
judgment in the loop. Because the apparatus decouples sim time from wall-clock,
the model's (slow, variable) API latency is free, and $r$ is engineered
exactly as in S1: Sonnet~4.6 judges \emph{every grounded frame} of a
closed-loop blocking episode (pit width 2, $v = 0.07$, fog 0, seeds 1--6), its
committed jump is applied $D$ ticks later, and sweeping
$D \in \{0, \dots, 31\}$ sweeps $r = vD/W$ from $0$ to $2$, densely around
$r = 1$ (72 episodes; 6{,}612 vision calls).

\paragraph{Judge protocol.} The episode prompt used for the S1 latency battery
turns out never to elicit JUMP near the window from \emph{any} hosted model:
it describes the pit as ``the black band across the floor ahead,'' which stops
matching the close-range view --- inside the window the pit is a black region
filling the bottom of the frame, and the visible checkered floor is the pit's
\emph{far} side. The S5 judge instead answers a literal perceptual question
(``what touches the bottom border of the image: tiles or black?'') at high
reasoning effort, with a commit rule of two consecutive JUMP answers. The
commit rule matters: in prompt-development batteries we measured isolated
false JUMPs at the 1--2\% per-call level far from the pit, and any such rate
fires almost surely somewhere over the $\sim$90 consulted approach frames ---
a first-passage failure mode that single-frame accuracy batteries cannot see.
We validated the final judge on all 108 approach frames $\times$ 3 samples:
zero far-field JUMPs, unanimous JUMP inside the window, and isolated misfires
only in the last half-tile before the window ($x = 8.10$--$8.24$) --- the
source of the 9/72 early commits below. The bottom-edge cue is nearly ideal in this
renderer: the pit's black region first touches the frame's bottom edge at
$x = 9.01$, one tick after the window opens ($w_{lo} = 8.92$).

\paragraph{The law holds.} Episodes whose decision lands in the window clear
\textbf{32/32 through $r = 0.97$}, 4/5 at $r = 1.04$, and \textbf{0/26 from
$r = 1.10$ on}; the same logistic fit as S1 gives $\rstar = 1.05$, inside the
oracle's bracketing gap $[1.04, 1.30]$ and at its last-clear edge (the
same-tick takeoff tolerance of S1). The transition is nearly as sharp as the
oracle's, for an unplanned reason: the model's commit position is almost
deterministic --- 51/72 episodes commit at $x = 8.94$ and 12 at $x = 9.01$,
straddling the cue's pixel-level onset (the model begins answering ``black''
one tick before the black region literally reaches the bottom edge at
$x = 9.01$). The anticipated noisy
psychometric curve compresses to a near-step; this judge's decision-position
variance is sub-tick, so the model behaves as a \emph{slightly-early oracle}
rather than a noisy one.

\paragraph{Delay-rescued misjudgments.} 9/72 episodes commit early
($x = 8.10$--$8.24$, out of window --- a perception error; these are excluded
from the timing fit above). At $D \le 8$ every such episode fails, landing
short. In the band $D = 15$--$24$ ($r = 0.97$--$1.56$), \emph{every} early episode
\textbf{clears} (5/5): the delay pushes the too-early takeoff into the window.
Past $r \approx 1.1$ the \emph{only} surviving episodes are the misjudged
ones; by $r = 2.01$ the delay overshoots even the early takeoff and the
rescue band closes. The two failure axes do not merely
dissociate --- on opposite sides of the boundary they \emph{compensate}, which
is only visible because the apparatus labels each decision in- or
out-of-window.

\begin{figure}[h]
  \centering
  \includegraphics[width=0.72\linewidth]{fig_realmodel.png}
  \caption{S5. Sonnet 4.6 swept through the boundary in closed loop
  ($\rstar = 1.05$, in-window decisions). Crosses are out-of-window (early)
  decisions: they fail below the boundary and are the only survivors past it
  (delay-rescued perception errors).}
\end{figure}
...
4--47$\times$ on the latter two. S5 is the direct demonstration: the same
model, same judgment, clears 32/32 below the boundary and 0/26 above it,
with nothing varied but $r$. The practical reading is that pausable or
...
S5's delay-rescued misjudgments add a sharper corollary: past the boundary,
scores can even \emph{reward} bad perception --- the axes compensate, and an
aggregate benchmark cannot tell.

\section{Limitations}

\begin{itemize}
  \item \textbf{The real-model sweep (S5) is one model at one pit width}, and
    its judge uses a revised prompt (the S1 battery prompt elicits no JUMPs
    near the window from any hosted model) plus a two-consecutive-JUMP commit
    rule --- a documented apparatus change, not the shipped default. Its $r$
    axis is engineered by injected delay with the model's judgment in the
    loop; the model's own wall-clock latency ($\sim$5\,s/call at high effort)
    remains far past $\rstar$, so a wall-clock-real sweep is still impossible
    for the measured field --- that is S1's point, not a gap. The
    \textbf{latency axis is real}: five models measured directly, all past
    $\rstar$ (hosted $r \approx 4$--$6$, local $r \approx 31$--$47$);
    network-/hardware-dependent, but real decision latencies, not stand-ins.
    The anticipated \emph{noisy} psychometric curve did not materialize ---
    this judge's decision variance is sub-tick --- so mapping a genuinely
    noisy transition needs a weaker or more stochastic judge.
...
Extending the S5 real-model sweep to more models and pit widths (and to a
...
psychophysics arm; and more reflex tasks (beyond the two here) to test how
universal the $r \approx 1$ boundary is.
...
latencies (\code{vlm\_latency.jsonl}, 100 rows across 5 models), the oracle
and aim sweeps, and the S5 real-model sweep
(\code{eval/pit/realmodel/}: driver, revised judge, 108-frame battery
validation, and the 72-episode JSONL) are committed. The local-VLM paths
(Ollama) and the Claude subagent judges run without an API key; the hosted-API
latency numbers and the S5 sweep required one.
