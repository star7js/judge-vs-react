...
commit rule matters: the model's rare stochastic false JUMPs
($\sim$2\% per call far from the pit) would otherwise fire almost surely
somewhere over the $\sim$90 consulted approach frames --- a first-passage
failure mode that single-frame accuracy batteries cannot see. We validated the
judge on all 108 approach frames $\times$ 3 samples: zero far-field JUMPs,
unanimous JUMP inside the window. The bottom-edge cue is nearly ideal in this
...
the first one or two ticks the bottom-edge cue exists. The anticipated noisy
